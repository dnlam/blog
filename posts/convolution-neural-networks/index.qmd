...
Then, we can create a state-of-the-art ResNet model using the stem and ResNet blocks as follows:

- There are four ResNet blocks with 64, 128, 256, and 512 output filters, respectively.

- Each group starts with a stride-2 block, exept the first group, which is after the MaxPooing layer.
...
```{python}
def _bottleneck_block(ni, nf, stride):
    return nn.Sequential(
        ConvLayer(ni, nf//4,  1),
...
And lastly, we can perform ResNet-50 in a dataset with larger images (e.g., 224x224 pixels) to see how well it performs. 

```{python}
dls = get_data(URLs.IMAGENETTE_160, 160, 224)
