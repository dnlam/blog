...
The results that we have achieved is great with ResNet-50 model built from scratch (around 90% accuracy on `Imagenette` dataset with 224x224 pixel images after 25 epochs of training). 
...
#| echo: false
def get_learner(model):
    learn = Learner(dls, model, loss_func=nn.CrossEntropyLoss(), metrics=accuracy, cbs=Mixup)
    return learn
learn = get_learner(rn50)
